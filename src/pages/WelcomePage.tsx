...
export const WelcomePage = () => {
  ...
  const switchMode = (m: 'create' | 'login') => {
    setMode(m); setName(''); setPass(''); setConfirm(''); setError('');
  };

  const handleCreate = async (e: FormEvent) => {
    e.preventDefault();
    const n = name.trim();
    if (n.length < 2)  return setError('Username must be at least 2 characters!');
    if (n.length > 20) return setError('Username must be 20 characters or less!');
    if (!/^[a-zA-Z0-9_-]+$/.test(n)) return setError('Username: only letters, numbers, _ and - allowed!');
    if (pass.length < 6) return setError('Password must be at least 6 characters!');
    if (!isAlphanumeric(pass)) return setError('Password: only letters and numbers allowed!');
  ...
  const inputStyle = (hasContent: boolean, invalid: boolean): React.CSSProperties => ({
    ...
    borderRadius: '12px', background: '#0d1117', color: '#fff',
    outline: 'none', textAlign: 'center' as const, letterSpacing: '1px',
  ...
  return (
    <div style={{
      ...
      <div style={{
        background: '#16213E', border: '4px solid #FFE234', borderRadius: '24px',
        padding: '32px 28px', maxWidth: '420px', width: '100%',
        boxShadow: '8px 8px 0px #000', textAlign: 'center',
      }}>
        <h1 style={{
          fontFamily: '"Bangers", cursive', fontSize: 'clamp(2.2rem, 7vw, 3.4rem)',
          color: '#FFE234', textShadow: '4px 4px 0px #000, -2px -2px 0px #FF8C00',
          letterSpacing: '4px', margin: '0 0 4px', lineHeight: 1,
        }}>⚔️ TYPE KNIGHT ⚔️</h1>
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '20px' }}>
          {(['create', 'login'] as const).map(m => (
            <button key={m} type="button" onClick={() => switchMode(m)} style={{
              flex: 1, padding: '10px', fontFamily: '"Bangers", cursive',
              fontSize: '1.1rem', letterSpacing: '2px', borderRadius: '10px',
          ...
          <>
            ...
            {isFlixbee && name.length > 0 && (
              <div style={{
                ...
                👑 Welcome, <strong>{FLIXBEE_USERNAME}</strong>! You get free Pro + <strong>{FLIXBEE_MONTHLY_TOKENS} tokens</strong>!
              </div>
            )}

            <form onSubmit={handleCreate} style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              <div>
                <div style={{ fontFamily: '"Fredoka One", cursive', color: '#aaa', fontSize: '0.82rem', marginBottom: '4px', textAlign: 'left' }}>Knight Name</div>
                <input type="text" value={name} onChange={e => { setName(e.target.value); setError(''); }}
                  placeholder="e.g. CoolKnight99" maxLength={20} autoFocus style={inputStyle(name.length > 0, false)} />
                <div style={{ fontFamily: '"Fredoka One", cursive', color: '#444', fontSize: '0.75rem', textAlign: 'right', marginTop: '2px' }}>{name.length}/20</div>
              </div>
              ...
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  <input type={showPass ? 'text' : 'password'} value={pass}
                    ...
                    placeholder="••••••" maxLength={32} style={inputStyle(pass.length > 0, passInvalid)} />
                ...
                {pass.length > 0 && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '6px' }}>
                    <div style={{ display: 'flex', gap: '3px' }}>
                      {[1,2,3,4].map(n => (
                        <div key={n} style={{ width: '24px', height: '6px', borderRadius: '3px', background: n <= strength.bars ? strength.color : '#333', transition: 'background 0.2s' }} />
                      ))}
                    </div>
                    <span style={{ fontFamily: '"Fredoka One", cursive', fontSize: '0.75rem', color: strength.color }}>{strength.label}</span>
                  </div>
                )}
              </div>

              <div>
                <div style={{ fontFamily: '"Fredoka One", cursive', color: '#aaa', fontSize: '0.82rem', marginBottom: '4px', textAlign: 'left' }}>Confirm Password</div>
                <input type={showPass ? 'text' : 'password'} value={confirm}
                  onChange={e => { setConfirm(e.target.value); setError(''); }}
                  placeholder="••••••" maxLength={32} style={inputStyle(confirm.length > 0, confirmInvalid)} />
                {confirm.length > 0 && !confirmInvalid && (
                  <div style={{ fontFamily: '"Fredoka One", cursive', fontSize: '0.75rem', color: '#34C759', marginTop: '2px', textAlign: 'right' }}>✓ Passwords match</div>
                )}
              </div>

              {error && (
                <div style={{ background: '#3a0a0a', border: '2px solid #EF4444', borderRadius: '10px', padding: '8px 12px', fontFamily: '"Fredoka One", cursive', color: '#FF6B6B', fontSize: '0.88rem' }}>
                  ❌ {error}
                </div>
              )}

              <button type="submit" disabled={!canCreate || loading} className="cartoon-btn" style={{
                width: '100%', fontSize: '1.2rem', padding: '13px', marginTop: '4px',
                background: !canCreate || loading ? '#2a2a4a' : isFlixbee ? '#34C759' : '#FFE234',
                color: !canCreate || loading ? '#555' : '#000',
                cursor: canCreate && !loading ? 'pointer' : 'not-allowed',
                border: `3px solid ${canCreate && !loading ? '#000' : '#444'}`,
              }}>
                {loading ? '⏳ Creating...' : isFlixbee ? '👑 ENTER, FLIXBEE!' : '⚔️ JOIN THE BATTLE!'}
              </button>
            </form>
          </>
        ) : (
          <>
            <h2 style={{ fontFamily: '"Bangers", cursive', fontSize: '1.5rem', color: '#fff', letterSpacing: '2px', margin: '0 0 4px' }}>WELCOME BACK!</h2>
            <p style={{ fontFamily: '"Fredoka One", cursive', color: '#666', fontSize: '0.85rem', margin: '0 0 14px' }}>
            ...
            <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              <div>
                <div style={{ fontFamily: '"Fredoka One", cursive', color: '#aaa', fontSize: '0.82rem', marginBottom: '4px', textAlign: 'left' }}>Knight Name</div>
                <input type="text" value={name} onChange={e => { setName(e.target.value); setError(''); }}
                  placeholder="Your username" maxLength={20} autoFocus style={inputStyle(name.length > 0, false)} />
              </div>

              <div>
                <div style={{ fontFamily: '"Fredoka One", cursive', color: '#aaa', fontSize: '0.82rem', marginBottom: '4px', textAlign: 'left' }}>Password</div>
                <div style={{ position: 'relative' }}>
                  <input type={showPass ? 'text' : 'password'} value={pass}
                    onChange={e => { setPass(e.target.value); setError(''); }}
                    placeholder="••••••" maxLength={32} style={inputStyle(pass.length > 0, false)} />
                  <button type="button" onClick={() => setShowPass(s => !s)} style={{
                    position: 'absolute', right: '10px', top: '50%', transform: 'translateY(-50%)',
                    background: 'none', border: 'none', cursor: 'pointer', fontSize: '1.1rem',
                  }}>{showPass ? '🙈' : '👁️'}</button>
              ...
              <button type="submit" disabled={!canLogin || loading} className="cartoon-btn" style={{
                width: '100%', fontSize: '1.2rem', padding: '13px', marginTop: '4px',
                background: !canLogin || loading ? '#2a2a4a' : '#a855f7',
                color: !canLogin || loading ? '#555' : '#fff',
                cursor: canLogin && !loading ? 'pointer' : 'not-allowed',
                border: `3px solid ${canLogin && !loading ? '#000' : '#444'}`,
              }}>
        ...
        <p style={{ fontFamily: '"Fredoka One", cursive', color: '#333', fontSize: '0.75rem', marginTop: '18px', marginBottom: 0 }}>
          Progress saved securely in the cloud ☁️
        </p>
      </div>
    </div>
  );
